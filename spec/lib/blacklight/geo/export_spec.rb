...
describe "BlacklightGeo::GeojsonExport" do
  ...
  subject {BlacklightGeo::GeojsonExport.new(@controller, @response.docs)}

  it "should instantiate GeojsonExport" do
    expect(subject.class).to eq(::BlacklightGeo::GeojsonExport)
  end

  it "should return maps config" do
    expect(subject.send(:blacklight_geo_config).class).to eq(::Blacklight::Configuration::ViewConfig)
  end
